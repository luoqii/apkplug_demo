...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//监听插件安装状态已动态更新列表
		ListenerBundleEvent();
		System.out.println("getMainLooper "+this.getMainLooper().getThread().getName());
		System.out.println("Thread Name "+Thread.currentThread().getName());
		/*FrameworkFactory.getInstance().init(this.getApplication());
		this.finish();
		
		Intent i=new Intent();
		i.setClassName(this, huahua.viewpager.MainActivity.class.getCanonicalName());
	...
	/**
	 ...
	 public void install(String path,installCallback callback) throws Exception{
		...
		//插件启动级别为1(会自启) 并且不检查插件版本是否相同都安装
		 agent.getService().install(mcontext, path,callback, 1,false);
	}
	/**
